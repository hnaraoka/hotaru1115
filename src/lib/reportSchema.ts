...
export const reportInputSchema = z
  .object({
    ...
    clientCompany: z.string().trim().min(1, "参画先企業は必須です"),
    workLocation: z.string().trim().min(1, "作業場所は必須です"),
    ...
    projectName: z.string().trim().min(1, "プロジェクト名は必須です"),
    ...
    workContent: z.string().trim().min(1, "作業内容は必須です"),
    devProcesses: z.array(z.enum(devProcessValues)).min(1, "開発工程を1つ以上選択してください"),

    deliverables: z.string().trim().optional().nullable(),
    troubles: z.string().trim().optional().nullable(),
    goodPoints: z.string().trim().optional().nullable(),
