  ...
  return z
    .object({
      ...
      clientCompany: z
        ...
        .trim()
        .min(1, "参画先企業は必須です")
        .max(200, "参画先企業は200文字以内で入力してください"),
      workLocation: z
        .string()
        .trim()
        .min(1, "作業場所は必須です")
        .max(200, "作業場所は200文字以内で入力してください"),
      ...
      devProcesses: z
        .array(z.enum(devProcessValues))
        .min(devProcessesRequired ? 1 : 0, "開発工程を1つ以上選択してください"),
...
export type ReportInput = z.infer<ReturnType<typeof buildReportInputSchema>>;
